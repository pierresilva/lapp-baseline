...
class InverseSeed
{
     ...
     * @return bool
     * @throws pierresilva\InverseSeed\TableNotFoundException
     */
    ...
    {
        return base_path() . config('inverse-seed::config.path');
    }
    ...
    {
        $chunkSize = $chunkSize ?: config('inverse-seed::config.chunk_size');
        $inserts = '';
    ...
    /**
     * Cleans the InverseSeed section
     * @return bool
     */
    public function cleanSection()
    {
        $databaseSeederPath = base_path() . config('inverse-seed::config.path') . '/DatabaseSeeder.php';

        $content = $this->files->get($databaseSeederPath);

        $content = preg_replace("/(\#inverse-seed_start.+?)\#inverse-seed_end/us", "#inverse-seed_start\n\t\t#inverse-seed_end", $content);
    ...
    public function updateDatabaseSeederRunMethod($className)
    {
        ...
        if (strpos($content, "\$this->call({$className}::class)") === false) {
            if (
                strpos($content, '#inverse-seed') &&
                strpos($content, '#inverse-seed') &&
                strpos($content, '#inverse-seed') < strpos($content, '#inverse-seed_end')
            ) {
                $content = preg_replace("/(\#inverse-seed_start.+?)(\#inverse-seed_end)/us", "$1\$this->call({$className}::class);{$this->newLineCharacter}{$this->indentCharacter}{$this->indentCharacter}$2", $content);
            } else {
