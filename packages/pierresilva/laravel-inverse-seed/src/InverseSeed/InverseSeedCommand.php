...
class InverseSeedCommand extends Command
{
    /**
     ...
     *
     * @return \Orangehill\InverseSeed\InverseSeed
     */
    ...
    {
        // if clean option is checked empty InverseSeed template in DatabaseSeeder.php
        if ($this->option('clean')) {
            app('InverseSeed')->cleanSection();
        }
    ...
    {
        return array(
            array('clean', null, InputOption::VALUE_NONE, 'clean inverse-seed section', null),
    ...
    protected function generateFileName($table)
    {
        ...
        $className = app('InverseSeed')->generateClassName($table);
        $seedPath = base_path() . config('inverse-seed::config.path');
