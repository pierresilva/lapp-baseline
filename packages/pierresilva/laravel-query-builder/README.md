...
```php
use pierresilva\QueryBuilder\QueryBuilder;
...
```bash
composer require pierresilva/laravel-query-builder
```

You can optionally publish the config file with:
```bash
php artisan vendor:publish --provider="pierresilva\QueryBuilder\QueryBuilderServiceProvider" --tag="config"
```
...
```php
use pierresilva\QueryBuilder\Filter;
...
Exact filters can be added using `pierresilva\QueryBuilder\Filter::exact('property_name')` in the `allowedFilters()` method.
...
You can specify custom filters using the `Filter::custom()` method. Custom filters are simple, invokable classes that implement the `\pierresilva\QueryBuilder\Filters\Filter` interface. This way you can create any query your heart desires.

For example:

``` php
use pierresilva\QueryBuilder\Filters\Filter;
...
If you want to completely adhere to the JSON API specification you can also use our own [pierresilva/json-api-paginate](https://github.com/pierresilva/laravel-json-api-paginate)!
