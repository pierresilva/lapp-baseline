...
```bash
composer require pierresilva/passport-custom-jwt-claims
```
...
as `PassportJwtClaimsServiceProvider` extends from it.

##  Configuration
To set your custom claims you must publish the config file:
                          
```bash
php artisan vendor:publish --provider="PassportJwtClaimsServiceProvider"
```
