...
pierresilva is a web design agency in Antwerp, Belgium. You'll find an overview of all
our open source projects [on our website](https://pierresilva.be/opensource).
...
This package can be used in Laravel 5.4 or higher. If you are using an older version of Laravel, take a look at [the v1 branch of this package](https://github.com/pierresilva/laravel-permission/tree/v1).

You can install the package via composer:

``` bash
composer require pierresilva/laravel-permission
```
...
'providers' => [
    // ...
    pierresilva\Permission\PermissionServiceProvider::class,
];
```

You can publish [the migration](https://github.com/pierresilva/laravel-permission/blob/master/database/migrations/create_permission_tables.php.stub) with:

```bash
php artisan vendor:publish --provider="pierresilva\Permission\PermissionServiceProvider" --tag="migrations"
```
...
```bash
php artisan vendor:publish --provider="pierresilva\Permission\PermissionServiceProvider" --tag="config"
```

When published, [the `config/permission.php` config file](https://github.com/pierresilva/laravel-permission/blob/master/config/permission.php) contains:
...
return [
    ...
    'models' => [
         ...
         * `pierresilva\Permission\Contracts\Permission` contract.
         */

        'permission' => pierresilva\Permission\Models\Permission::class,
         ...
         * `pierresilva\Permission\Contracts\Role` contract.
         */

        'role' => pierresilva\Permission\Models\Role::class,
    ...
    'cache' => [
        ...
        'key' => 'pierresilva.permission.cache',
...
```bash
mkdir -p config
cp vendor/pierresilva/laravel-permission/config/permission.php config/permission.php
cp vendor/pierresilva/laravel-permission/database/migrations/create_permission_tables.php.stub database/migrations/2018_01_01_000000_create_permission_tables.php
```
...
$app->routeMiddleware([
    ...
    'permission' => pierresilva\Permission\Middlewares\PermissionMiddleware::class,
    'role'       => pierresilva\Permission\Middlewares\RoleMiddleware::class,
]);
```
...
$app->register(pierresilva\Permission\PermissionServiceProvider::class);
```
...
First, add the `pierresilva\Permission\Traits\HasRoles` trait to your `User` model(s):

```php
use Illuminate\Foundation\Auth\User as Authenticatable;
use pierresilva\Permission\Traits\HasRoles;
...
```php
use pierresilva\Permission\Models\Role;
use pierresilva\Permission\Models\Permission;
...
The `role` scope can accept a string, a `\pierresilva\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
The scope can accept a string, a `\pierresilva\Permission\Models\Permission` object or an `\Illuminate\Support\Collection` object.
...
The `assignRole`, `hasRole`, `hasAnyRole`, `hasAllRoles`  and `removeRole` functions can accept a
 string, a `\pierresilva\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
string or a `pierresilva\Permission\Models\Permission` object.
...
All these responses are collections of `pierresilva\Permission\Models\Permission` objects.
...
See this wiki article on [Defining a Super-Admin Gate rule](https://github.com/pierresilva/laravel-permission/wiki/Global-%22Admin%22-role) in your app.
...
protected $routeMiddleware = [
    ...
    'role' => \pierresilva\Permission\Middlewares\RoleMiddleware::class,
    'permission' => \pierresilva\Permission\Middlewares\PermissionMiddleware::class,
    'role_or_permission' => \pierresilva\Permission\Middlewares\RoleOrPermissionMiddleware::class,
];
```
...
{
    if ($exception instanceof \pierresilva\Permission\Exceptions\UnauthorizedException) {
        // Code here ...
    }
...
```php
    public function setUp()
    {
        ...
        $this->app->make(\pierresilva\Permission\PermissionRegistrar::class)->registerPermissions();
    }
```
...
class RolesAndPermissionsSeeder extends Seeder
{
    public function run()
    {
        ...
        app()[\pierresilva\Permission\PermissionRegistrar::class]->forgetCachedPermissions();
...
- Your `Role` model needs to extend the `pierresilva\Permission\Models\Role` model
- Your `Permission` model needs to extend the `pierresilva\Permission\Models\Permission` model

If you need to REPLACE the existing `Role` or `Permission` models you need to keep the
following things in mind:

- Your `Role` model needs to implement the `pierresilva\Permission\Contracts\Role` contract
- Your `Permission` model needs to implement the `pierresilva\Permission\Contracts\Permission` contract
...
```php
app()->make(\pierresilva\Permission\PermissionRegistrar::class)->forgetCachedPermissions();
```
...
- [Extensive tutorial for building permissions UI](https://scotch.io/tutorials/user-authorization-in-laravel-54-with-pierresilva-laravel-permission) by [Caleb Oki](http://www.caleboki.com/).
