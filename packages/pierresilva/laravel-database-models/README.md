...
You will need to import the `pierresilva/laravel-database-models` package via composer:

```shell
composer require pierresilva/laravel-database-models
```
...
'providers' => [
    ...
    pierresilva\DatabaseModelsServiceProvider::class,
],
// ...
```
...
public function register()
{
    if ($this->app->environment() == 'local') {
        $this->app->register(\pierresilva\DatabaseModelsServiceProvider::class);
    }
}
```
...
```shell
php artisan vendor:publish --tag=database-models
...
```shell
php artisan database:models
```

- You can scaffold a specific table like this:

```shell
php artisan database:models --table=users
```

- You can also specify the connection:

```shell
php artisan database:models --connection=mysql
```

- If you are using a MySQL database, you can specify which schema you want to scaffold:

```shell
php artisan database:models --schema=shop
```

### Customizing Model Scaffolding

To change the scaffolding behaviour you can make `config/database-models.php` configuration file
